...
async def run_demo():
    ...
    # 1. 用户提出模糊需求
    print("User: 我想去日本玩。")
    async for event in travel_agent.astream({"messages": [HumanMessage(content="我想去日本玩")]}, config):
    ...
    async for event in travel_agent.astream({"messages": [HumanMessage(content="从上海出发，下周五走，预算 1万左右")]}, config):
        pass
    ...
    # 3. 突然插入闲聊 (侧轨测试)
    print("\nUser: 日本最近冷吗？")
    async for event in travel_agent.astream({"messages": [HumanMessage(content="日本最近冷吗？")]}, config):
        pass
    ...
    async for event in travel_agent.astream({"messages": [HumanMessage(content="还是改去泰国吧，预算加到 1.5万。")]}, config):
        pass
    ...
    # 5. 选择方案 (回到主流程)
    print("\nUser: 选方案 2。")
    async for event in travel_agent.astream({"messages": [HumanMessage(content="选方案 2")]}, config):
        pass
    ...
    # 7. 模拟支付回调
    print("\n[System]: 支付回调收到，继续...")
    async for event in travel_agent.astream(None, config):
        pass
